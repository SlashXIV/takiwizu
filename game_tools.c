  ...
  for (row = 0; row < game_nb_rows(g); row++) {
    for (col = 0; col < game_nb_cols(g); col++) {
      if (empty_square(game_get_square(g, row, col))){
        ...
        // Backtrack again and return false if the current square cannot be filled with either a white or a black square
        square_clear(g, row, col);
        return false;
      }
    }
  }
...
bool game_solve(game ga) {
  ...
  for (row = 0; row < game_nb_rows(g); row++) {
    ...
    for (col = 0; col < game_nb_cols(g); col++) {
      if (empty_square(game_get_square(g, row, col))) {
        ...
        square_clear(g, row, col);
        return false;
      } else if (game_get_square(g, row, col) == S_ONE) {
        black_count_row++;
      } else if (game_get_square(g, row, col) == S_ZERO) {
        white_count_row++;
      }
    }
    if (black_count_row > game_nb_cols(g) / 2 || white_count_row > game_nb_cols(g) / 2) {
      return false;
    }
  }
  ...
  for (col = 0; col < game_nb_cols(g); col++) {
    ...
    if (black_count_col > game_nb_rows(g) / 2 || white_count_col > game_nb_rows(g) / 2) {
      return false;
    }
  }
  printf("Je cherche solution\n");

  //put all in a file with this format : 010101|011010|100101|101001|010101|100110
...
void generate_first_solution(int pos, int len, uint *count, bool solved, game solution, game g)
{
...
bool game_solve(game g){
  ...
  bool solved = true;
  uint count=0;
  generate_first_solution(0, game_nb_cols(g) * game_nb_rows(g), &count, solved, game_solved, g);

  //boucle for pour récupérer les valeurs des cases dans game_solved et les mettre dans un fichier
